...
type
  ...
  var //fields can't appear after method, let's use vars instead
    ...
    procedure VScr_AreaMouseLeave(Sender: TObject);
    procedure VScr_AreaMouseMove(Sender: TObject; {%H-}Shift: TShiftState;
      X, Y: integer);
    procedure VScr_AreaRedraw(Sender: TObject; Bitmap: TBGRABitmap);
  private
...
begin
  ...
  VScr_Area := TBGRAVirtualScreen.Create(Self);
  VScr_Area.AutoSize := True;
  VScr_Area.AnchorSideLeft.Control := self;//self is instance of tform_1
  VScr_Area.AnchorSideTop.Control := Pnl_1;
  VScr_Area.AnchorSideTop.Side := asrBottom;
  VScr_Area.AnchorSideRight.Control := self;//self is instance of tform_1
  VScr_Area.AnchorSideRight.Side := asrBottom;
  VScr_Area.AnchorSideBottom.Control := self;//self is instance of tform_1
  VScr_Area.AnchorSideBottom.Side := asrBottom;
  VScr_Area.Anchors := [akTop, akLeft, akRight, akBottom];
  //VScr_Area.BevelColor := $0000FF4D;
  //VScr_Area.BevelOuter := bvSpace;
  //VScr_Area.BevelWidth := 5;
  VScr_Area.ParentColor := False;
  VScr_Area.TabOrder := 0;
  VScr_Area.OnRedraw := @VScr_AreaRedraw;
  VScr_Area.OnMouseLeave := @VScr_AreaMouseLeave;
  VScr_Area.OnMouseMove := @VScr_AreaMouseMove;
  VScr_Area.Parent := self;  //self is instance of tform_1
...
begin
  ...
  begin
    ...
    html += VScr_Area.Bitmap.Canvas2D.toDataURL;
    html += '"/></body></html>';
...
procedure Tform_1.VScr_AreaMouseLeave(Sender: TObject);
begin
  mx := -1000;
  my := -1000;
end;

procedure Tform_1.VScr_AreaMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: integer);
begin
  mx := X;
  my := Y;
...
procedure Tform_1.VScr_AreaRedraw(Sender: TObject; Bitmap: TBGRABitmap);
var
  ctx: TBGRACanvas2D;
  grainElapse: integer;
  newTime: TDateTime;
